...
/**
 * This is a small modification of {@link Tutorial4} using a rectangular area light
...
public class Tutorial4_2 {
    ...
    private void init() throws IOException {
        ...
        glfwSetFramebufferSizeCallback(window, fbCallback = new GLFWFramebufferSizeCallback() {
            public void invoke(long window, int width, int height) {
                if (width > 0 && height > 0 && (Tutorial4_2.this.width != width || Tutorial4_2.this.height != height)) {
                    Tutorial4_2.this.width = width;
                    Tutorial4_2.this.height = height;
                    Tutorial4_2.this.resetFramebuffer = true;
                    /*
                     * Reset the frame counter. Any change in framebuffer size will reset the
                     * current accumulated result.
                     */
                    Tutorial4_2.this.frameNumber = 0;
                }
            }
        });
        ...
        glfwSetCursorPosCallback(window, cpCallback = new GLFWCursorPosCallback() {
            ...
            public void invoke(long window, double x, double y) {
                if (mouseDown) {
                    float deltaX = (float) x - Tutorial4_2.this.mouseX;
                    float deltaY = (float) y - Tutorial4_2.this.mouseY;
                    Tutorial4_2.this.viewMatrix.rotateLocalY(deltaX * 0.01f);
                    Tutorial4_2.this.viewMatrix.rotateLocalX(deltaY * 0.01f);
                    /*
                ...
                Tutorial4_2.this.mouseX = (float) x;
                Tutorial4_2.this.mouseY = (float) y;
            }
        });
        ...
        glfwSetMouseButtonCallback(window, mbCallback = new GLFWMouseButtonCallback() {
            public void invoke(long window, int button, int action, int mods) {
                if (action == GLFW_PRESS) {
                    Tutorial4_2.this.mouseDown = true;
                } else if (action == GLFW_RELEASE) {
                    Tutorial4_2.this.mouseDown = false;
                }
            }
        });
    ...
    private void createQuadProgram() throws IOException {
        ...
        int vshader = DemoUtils.createShader("org/lwjgl/demo/opengl/raytracing/tutorial4_2/quad.vs.glsl", GL_VERTEX_SHADER);
        int fshader = DemoUtils.createShader("org/lwjgl/demo/opengl/raytracing/tutorial4_2/quad.fs.glsl", GL_FRAGMENT_SHADER);
    ...
    private void createComputeProgram() throws IOException {
         ...
         */
        int program = glCreateProgram();
        int random = DemoUtils.createShader("org/lwjgl/demo/opengl/raytracing/tutorial4_2/random.glsl", GL_COMPUTE_SHADER);
        int cshader = DemoUtils.createShader("org/lwjgl/demo/opengl/raytracing/tutorial4_2/raytracing.glsl", GL_COMPUTE_SHADER);
